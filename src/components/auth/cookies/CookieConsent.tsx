...
export const CookieConsentProvider: React.FC<{ 
  ...
  useEffect(() => {
    if (initialCheck) {
      if (consentGiven) {
        // Calcula a data de expiração usando o validityDays passado por prop
        ...
        localStorage.setItem("cookieConsent", JSON.stringify({
          ...
          validityDays: validityDays // Salva também o período de validade usado
        }));
      } else {
        localStorage.removeItem("cookieConsent");
      }
    }
  }, [consentGiven, initialCheck, validityDays]);
  
  return (
    <CookieConsentContext.Provider value={{ consentGiven, setConsentGiven, validityDays }}>
      {children}
...
const CookieSettingsModal: React.FC<CookieSettingsModalProps> = ({ isOpen, onClose, onAccept }) => {
  const { validityDays } = useCookieConsent();
...
const ProtectedContent: React.FC<{ 
  ...
  const handleAccept = () => {
    ...
    // Call onConsent callback with preferences
    if (onConsent) {
...
export default function CookieConsent({ children, onConsent, validityDays = 7 }: CookieConsentProps) {
  return (
    <CookieConsentProvider validityDays={validityDays}>
      <ProtectedContent onConsent={onConsent}>
        {children}
      </ProtectedContent>
