              ...
              {items.map((item) => {
                ...
                return (
                  <Link
                    key={item.title}
                    to={item.url}
                    className={`relative flex items-center gap-2 ${isLargeScreen ? "px-4 py-2.5" : "px-3 py-2"} text-sm font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                      isActive
            ...
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="profile"
                    className={`flex items-center gap-1 sm:gap-2 ${isMobile ? "px-2 py-1.5" : "px-3 py-2"} rounded-xl transition-all duration-200 hover:scale-105 active:scale-95 min-w-0`}
                  >
                    <div className={`${getAvatarSize()} overflow-hidden rounded-full ring-2 ring-gray-200 dark:ring-gray-700 flex-shrink-0`}>
                ...
                <DropdownMenuContent side="bottom" align="end" className={`${isMobile ? "w-64" : isTablet ? "w-68" : "w-72"} bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg border border-gray-200/50 dark:border-gray-700/50 rounded-xl shadow-xl p-2 mt-2`}>
                  ...
                  <DropdownMenuItem className="p-0 outline-none">
                    <Button
                      ...
                      className={`w-full justify-start gap-3 ${isMobile ? "p-2.5" : "p-3"} text-red-600 hover:text-red-700 hover:bg-red-50/80 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/20 rounded-lg transition-all duration-200`}
                    >
              ...
              <Button
                variant="ghost"
                size="sm"
                className={`lg:hidden ${isMobile ? "p-1.5" : "p-2"} rounded-xl hover:bg-gray-100/80 dark:hover:bg-gray-800/80 transition-all duration-200 active:scale-95`}
                ...
                {items.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.title}
                      to={item.url}
                      className={`relative flex items-center gap-3 ${isMobile ? "px-3 py-2.5" : "px-4 py-3"} rounded-xl transition-all duration-200 active:scale-95 ${
                        isActive
                    ...
                    >
                      <item.icon className={`${isMobile ? "w-4 h-4" : "w-5 h-5"} flex-shrink-0 ${isActive ? "animate-pulse" : ""}`} />
