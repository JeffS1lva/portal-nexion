...
export const PedidosFilter = ({
  ...
  const [dateFrom, setDateFrom] = React.useState<Date | undefined>(activeDateRange.start)
  const [dateTo, setDateTo] = React.useState<Date | undefined>(activeDateRange.end)
  ...
  const getPlaceholder = () => {
    switch (searchType) {
      ...
      case "pedidosCompra":
        return "Buscar por número de Pedidos de Compra..."
      case "statusDoPedido":
      ...
      <div className="flex flex-col space-y-4 py-2">
        <div className="flex flex-col space-y-3 md:flex-row md:items-start md:space-y-0 md:space-x-2">
            ...
            <div className="flex flex-col space-y-2 w-full md:flex-row md:flex-1 md:space-y-0 md:space-x-2">
              <Popover open={isFromPopoverOpen} onOpenChange={setIsFromPopoverOpen}>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <CustomCalendar selected={dateFrom} onSelect={handleFromDateChange} initialFocus />
                </PopoverContent>
              </Popover>
              ...
              <Popover open={isToPopoverOpen} onOpenChange={setIsToPopoverOpen}>
                ...
                <PopoverContent className="w-auto p-0" align="end">
                  <CustomCalendar selected={dateTo} onSelect={handleToDateChange} initialFocus />
                </PopoverContent>
              </Popover>
            </div>
          )}
          ...
          >
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem value="dataLancamentoPedido">Data de Lançamento</SelectItem>
                <SelectItem value="dataParaEntrega">Data de Entrega</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>

          <Select value={currentPeriodFilter} onValueChange={(value) => applyPeriodFilter(value as PeriodFilter)}>
          ...
          <div className="flex flex-col space-y-2 w-full md:flex-row md:w-auto md:space-y-0 md:space-x-2">
            {searchType === "dataLancamentoPedido" || searchType === "dataParaEntrega" ? (
              <Button onClick={handleApplyDateFilter} disabled={!dateFrom || !dateTo} className="w-full md:w-auto">
                Filtrar
              </Button>
            ) : (
              <Button
            ...
            <Button variant="outline" onClick={handleReset} className="w-full md:w-auto bg-transparent">
              Limpar
            </Button>
          </div>
        </div>
